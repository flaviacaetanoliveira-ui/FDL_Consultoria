...
def _render_kpis_principais(breakdown: LucroPresumidoBreakdown) -> None:
    ...
    with c2:
        ...
        fed = html.escape(_fmt_brl(breakdown.total_federal))
        est = html.escape(_fmt_brl(breakdown.total_estadual))
        st.markdown(
            '<div class="fdl-lp-metric-sublabel">Federal '
            f'<span style="font-variant-numeric: tabular-nums;">{fed}</span> · Estadual '
            f'<span style="font-variant-numeric: tabular-nums;">{est}</span></div>',
            unsafe_allow_html=True,
        )
    with c3:
